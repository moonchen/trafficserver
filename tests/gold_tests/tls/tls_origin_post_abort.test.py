...
class TestOriginPostAbort:
    ...
    def _configure_trafficserver(self) -> 'Process':
        ...
        ts.Disk.remap_config.AddLine('map /post https://127.0.0.1:{0}'.format(Test.Variables.origin_port))
    ...
    def run(self) -> None:
        ...
        tr.Processes.Default.Command = 'python3 {0} -p {1} -t 8'.format(
            os.path.join(Test.TestDirectory, 'tls_post_abort_client.py'), self._ts.Variables.port)

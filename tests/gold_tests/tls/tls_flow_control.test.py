...
class TestTlsFlowControl:
    ...
    def _configure_server(self) -> 'Process':
        ...
        response_header = {
            "headers":
                "HTTP/1.1 200 OK\r\nServer: microserver\r\nConnection: close\r\n"
                "Content-Length: {0}\r\n\r\n".format(TestTlsFlowControl._body_len),
...
""".split("\n"))
        ts.Disk.remap_config.AddLine('map / http://127.0.0.1:{0}'.format(self._server.Variables.Port))
    ...
    def run(self) -> None:
        ...
        tr.Processes.Default.Command = (
            "python3 {script} -p {port} --host ex.test --path /obj --expect-bytes {n}".format(
                script=os.path.join(Test.TestDirectory, 'tls_flow_control_client.py'),
                port=self._ts.Variables.ssl_port,
                n=TestTlsFlowControl._body_len))
        tr.Processes.Default.ReturnCode = 0
        tr.Processes.Default.Streams.All = Testers.ContainsExpression(
            "RESULT=PASS", "the full body must be delivered under flow control")
        tr.Processes.Default.Streams.All += Testers.ContainsExpression(
            "BODY_BYTES={0}".format(TestTlsFlowControl._body_len), "every body byte must arrive (no flow-control stall)")

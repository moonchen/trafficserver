...
""".split("\n"))
        ...
        ts.Disk.remap_config.AddLine('map / http://127.0.0.1:{0}'.format(self._server.Variables.http_port))
    ...
    def run(self) -> None:
        ...
        tr.Processes.Default.Command = "python3 {0} -p {1} -s example.com".format(
            os.path.join(Test.TestDirectory, 'tls_renegotiation_allowed_client.py'), self._ts.Variables.ssl_port)
        ...
        tr.MakeCurlCommand(
            (
                ...
                "--resolve 'example.com:{0}:127.0.0.1' https://example.com:{0}/").format(self._ts.Variables.ssl_port),
            ts=self._ts)
        tr.Processes.Default.ReturnCode = 0

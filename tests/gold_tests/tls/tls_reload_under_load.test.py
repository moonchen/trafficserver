...
""".split("\n"))
    ...
    def run(self) -> None:
        ...
        tr.Processes.Default.Command = (
            "python3 {script} -p {port} --sni bar.com --ssldir {ssldir} "
            "--live-cert signed-bar.pem --v2-cert {v2} --reloads 3 --duration 8 --concurrency 4".format(
                script=os.path.join(Test.TestDirectory, 'tls_reload_under_load_client.py'),
                port=self._ts.Variables.ssl_port,
                ssldir=self._ts.Variables.SSLDir,
                v2=os.path.join(self._ts.Variables.SSLDir, 'signed2-bar.pem')))

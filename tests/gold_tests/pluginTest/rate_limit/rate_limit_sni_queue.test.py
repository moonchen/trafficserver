...
reserve() trips a release assertion, aborting the server. Verified under ASan with the
allocator freelists disabled. ATS must survive the queue churn.
'''
...
class RateLimitSniQueueTest:
    ...
    def __init__(self) -> None:
        tr = Test.AddTestRun('rate_limit SNI queue accounting under ASan')
    ...
    def _configure_trafficserver(self) -> None:
        ...
        # Disable the freelist / ProxyAllocator so allocation behavior is not a confound and
        # ASan sees real allocations; the abort under test is a release-assertion, not an ASan
        # report, but this keeps the run representative of the -f -F CI config.
        ts.Command += ' -f -F'
        ...
        # Ignore the well-known benign ATS-startup leaks (Log::create_threads etc.).
        ts.Env['ASAN_OPTIONS'] = 'detect_leaks=0:detect_odr_violation=0'

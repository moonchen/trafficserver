...
cleanly (no use-after-free or crash), verified under ASan with the allocator
freelists disabled.
'''
...
class RateLimitSniRejectTest:
    ...
    def __init__(self) -> None:
        tr = Test.AddTestRun('rate_limit SNI reject teardown under ASan')
    ...
    def _configure_trafficserver(self) -> None:
        ...
        # free()'d rather than recycled; otherwise ASan cannot see a stale-VC access.
        ts.Command += ' -f -F'
        ...
        # Ignore the well-known benign ATS-startup leaks (Log::create_threads etc.) so
        # they cannot mask the use-after-free signal this test is watching for.

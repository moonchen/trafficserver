...
//
void
TCPNetVConnection::_read_reschedule(ink_hrtime delay)
{
  ink_release_assert(this_ethread() == thread);
  ink_assert(handler == &TCPNetVConnection::mainEvent);
  _read.state = op_state::TRY_ISSUE;
  thread->schedule_in_local(this, delay);
}

void
TCPNetVConnection::_write_reschedule(ink_hrtime delay)
{
  ink_release_assert(this_ethread() == thread);
  ink_assert(handler == &TCPNetVConnection::mainEvent);
...
TCPNetVConnection::_read_from_net(int event, Event *e)
{
  ...
  if (!lock.is_locked()) {
    _read_reschedule(HRTIME_MSECONDS(net_retry_delay));
    return EVENT_CONT;
  };
  ...
  // It's okay to let go of the lock while the recvmsg is happening, since the
  // structure of MIOBuffer is not updated during the write.
...
TCPNetVConnection::onRecvmsg(ssize_t bytes, std::unique_ptr<struct msghdr> msg, NetAIO::TCPConnection &c)
{
  ...
  Debug(TAG, "onRecvmsg: bytes = %zd", bytes);
...
TCPNetVConnection::onError(NetAIO::ErrorSource source, int err, NetAIO::TCPConnection &c)
{
  ...
  Debug(TAG, "onError: %d %d", static_cast<int>(source), err);
  ...
  if (source == NetAIO::ErrorSource::ES_RECVMSG) {
    lerrno      = err;
    _read.state = op_state::ERROR;
    _handle_read_error();
  }

  if (source == NetAIO::ErrorSource::ES_SENDMSG) {
    lerrno       = err;
    _write.state = op_state::ERROR;
    _handle_write_error();
    return;
  }
...
TCPNetVConnection::_write_to_net(int event, Event *e)
{
  ...
  if (!lock.is_locked() || lock.get_mutex() != _write.vio.mutex.get()) {
    _write_reschedule(HRTIME_MSECONDS(net_retry_delay));
    return EVENT_CONT;
  }
...
TCPNetVConnection::onSendmsg(ssize_t bytes, std::unique_ptr<struct msghdr> msg, NetAIO::TCPConnection &c)
{
  ...
  Debug(TAG, "onSendMsg: bytes = %zd", bytes);
...
TCPNetVConnection::_handle_write_done()
{
  ...
  if (r > 0) {
    ...
    Metrics::increment(net_rsb.write_bytes, r);
  ...
  if (!_write.signalled || (_write.vio.ntodo() > 0 && !buf.writer()->high_water())) {
    ...
    // @a signalled means we won't send an event, and the event values differing
    // means we had a write buffer trap and cleared it, so we need to send it
    // now.
    event_out = wbe_event;
  }
...
TCPNetVConnection::reenable(VIO *vio)
{
  ...
  if (vio == &_read.vio) {
    // Only issue if we were previously disabled
    if (_read.state == op_state::IDLE) {
      _read.state = op_state::TRY_ISSUE;
    }
  } else if (vio == &_write.vio) {
    // Only issue if we were previously disabled
    if (_write.state == op_state::IDLE) {
      _write.state = op_state::TRY_ISSUE;
    }
  } else {
    ink_release_assert(!"invalid vio!");
  }
  t->schedule_imm_local(this);
}
...
TCPNetVConnection::TCPNetVConnection() : _connect_state(connect_state::NONE)
{
  _read.vio.disable();
  _write.vio.disable();
...
void
TCPNetVConnection::setRemote(const sockaddr *remote)
{
  got_remote_addr = true;
  remote_addr.assign(remote);
}
...
{
  thread = t;
  ink_release_assert(this_ethread() == t);
  SET_HANDLER(&TCPNetVConnection::connectEvent);
  _con.emplace(remote_addr, &options, *get_PollDescriptor(t), *this);
...
TCPNetVConnection::onConnect(NetAIO::TCPConnection &c)
{
  ...
  // Fixme: call the handler in-line to work around state machine problems
  // _handle_connect_done();
  //
}
...
TCPNetVConnection::set_tcp_congestion_control(int side)
{
  ...
  if (!ccp.empty()) {
    int rv = setsockopt(_con->get_fd(), IPPROTO_TCP, TCP_CONGESTION, reinterpret_cast<const void *>(ccp.data()), ccp.size());

    if (rv < 0) {
      Error("Unable to set TCP congestion control on socket %d to \"%s\", "
            "errno=%d (%s)",
            _con->get_fd(), ccp.data(), errno, strerror(errno));
    } else {
      Debug("socket", "Setting TCP congestion control on socket [%d] to \"%s\" -> %d", _con->get_fd(), ccp.data(), rv);
    }
    return 0;
  }
  return -1;

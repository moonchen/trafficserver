...
public:
  ...
  void setRemote(const sockaddr *remote);
  ...
  // FIXME: these are not used, but are used to maintain compatibility with
  // UnixNetVConnection
...
private:
  ...
  void _read_reschedule(ink_hrtime delay);
  ...
  void _write_reschedule(ink_hrtime delay);
  ...
  int _recursion = 0;
  std::optional<NetAIO::TCPConnection> _con;
};

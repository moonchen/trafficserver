...
TCPListener::_setup_fd_for_listen()
{
  ...
  if (ats_is_ip6(&_local) && setsockopt_on(_fd, IPPROTO_IPV6, IPV6_V6ONLY) < 0) {
    goto Lerror;
  }

  if (setsockopt_on(_fd, SOL_SOCKET, SO_REUSEADDR) < 0) {
    goto Lerror;
  }

  if (setsockopt_on(_fd, SOL_SOCKET, SO_REUSEPORT) < 0) {
    goto Lerror;
  }
  ...
  if ((_opt.sockopt_flags & NetVCOptions::SOCK_OPT_NO_DELAY) && setsockopt_on(_fd, IPPROTO_TCP, TCP_NODELAY) < 0) {
    goto Lerror;
  }

  // enables 2 hour inactivity probes, also may fix IRIX FIN_WAIT_2 leak
  if ((_opt.sockopt_flags & NetVCOptions::SOCK_OPT_KEEP_ALIVE) && setsockopt_on(_fd, SOL_SOCKET, SO_KEEPALIVE) < 0) {
    goto Lerror;
  }

#ifdef TCP_FASTOPEN
  if ((_opt.sockopt_flags & NetVCOptions::SOCK_OPT_TCP_FAST_OPEN) && setsockopt_on(_fd, IPPROTO_TCP, TCP_FASTOPEN) < 0) {
  ...
  if (_opt.f_inbound_transparent) {
    ...
    if (setsockopt_on(_fd, SOL_IP, TS_IP_TRANSPARENT) < 0) {
  ...
  if (_opt.f_mptcp) {
#if MPTCP_ENABLED
    if (setsockopt_on(_fd, IPPROTO_TCP, MPTCP_ENABLED) < 0) {

...
public:
  ...
  explicit TCPConnection(const IpEndpoint &remote, const NetVCOptions *opt, PollDescriptor &pd, TCPConnectionObserver &observer,
                         int fd = NO_FD);
  ...
  // Attempt to read nbytes from the connection.  The format of msg is the same
  // as for recvmsg.  onRecvmsg will be called when the read makes progress.
  //
  // Ownership of msg is transferred to the TCPConnection.  The TCPConnection
  // will transfer ownership back when the read completes. You should either use
  // std::move to pass msg, or construct a temporary std::unique_ptr<struct
  // msghdr> and pass that.
  //
  // Returns true if the read can proceed.  False if the read cannot proceed
  // because the connection is not open, or another read is outstanding.
  bool recvmsg(std::unique_ptr<struct msghdr> msg, int flags);

  // Attempt to write.  The format of msg is the same as for sendmsg.  onSendmsg
  // be called when the write makes progress.
  //
  // Ownership of msg is transferred to the TCPConnection.  The TCPConnection
  // will transfer ownership back when the read completes. You should either use
  // std::move to pass msg, or construct a temporary std::unique_ptr<struct
  // msghdr> and pass that.
  //
  // Returns true if the write can proceed.  False if the write cannot proceed
  // because the connection is not open, or another write is outstanding.

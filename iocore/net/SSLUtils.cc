...
SSLMultiCertConfigLoader::load(SSLCertLookup *lookup, bool firstLoad)
{
  ...
  if (params->configLoadConcurrency > 0) {
    int num_threads                        = firstLoad ? std::thread::hardware_concurrency() : params->configLoadConcurrency;
    ...
    while (num_lines > 0) {
      ...
      threads.push_back(std::async(std::launch::async, &SSLMultiCertConfigLoader::_load_lines, this, lookup, current, last));
      current = last;
    }

    bool saw_error = false;
    // Wait for all futures to finish their tasks.
    ...
    if (!this->_load_lines(lookup, single_lines.begin(), single_lines.end())) {
      return false;
    }
  }

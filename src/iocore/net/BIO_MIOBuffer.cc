...
{
  // Throwing operator new cannot return null; let bad_alloc propagate as elsewhere.
...
void
miobuffer_consume(BIO *bio, int64_t nbytes)
{
  if (bio == nullptr || nbytes <= 0) {
    return;
  }
  BIO_MIOBuffer_Context *ctx = static_cast<BIO_MIOBuffer_Context *>(BIO_get_data(bio));
  if (ctx != nullptr && ctx->reader != nullptr) {
    ctx->reader->consume(nbytes);
  }
}
...
// BoringSSL (and other libraries lacking the BIO_meth_set_*_ex setters) use the
// classic int-based BIO read/write callbacks. Wrap the _ex implementations so the
// retry-flag and error semantics are preserved: a non-success _ex result (0) maps
// to the classic "-1" return, with the retry flags the _ex path already set
// distinguishing a would-block from a hard error.
static int
miobuffer_write(BIO *bio, const char *in, int inlen)
{
  if (inlen < 0) {
    return -1;
  }
  size_t written = 0;
  if (miobuffer_write_ex(bio, in, static_cast<size_t>(inlen), &written) <= 0) {
    return -1;
  }
...
static int
miobuffer_read(BIO *bio, char *out, int outlen)
{
  if (outlen < 0) {
    return -1;
  }
  size_t readbytes = 0;
  if (miobuffer_read_ex(bio, out, static_cast<size_t>(outlen), &readbytes) <= 0) {
    return -1;
  }
...
miobuffer_ctrl(BIO *bio, int cmd, [[maybe_unused]] long num, [[maybe_unused]] void *ptr)
{
  ...
  case BIO_CTRL_GET_CLOSE:
    // Owning/closing the underlying MIOBuffer is not supported; always report BIO_NOCLOSE.
    ret = 0;
    break;
  default:
...
bool
miobuffer_has_read_avail(BIO *bio)
{
...
#else
  BIO_meth_set_write(methods, miobuffer_write);
  BIO_meth_set_read(methods, miobuffer_read);
#endif

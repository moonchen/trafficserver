...
ServerSessionPool::eventHandler(int event, void *data)
{
  ...
  if (!found) {
    // We failed to find our session.  This can only be the result of a programming flaw.
    char peer_ip[INET6_ADDRPORTSTRLEN];
    ats_ip_nptop(net_vc->get_remote_addr(), peer_ip, sizeof(peer_ip));

    Warning("Connection leak from http keep-alive system fd=%d peer_ip_port=%s", net_vc->get_socket(), peer_ip);
    ink_assert(0);
  }
  return 0;
}

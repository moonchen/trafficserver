...
public:
  ...
  // (e.g. TLS ciphertext staged for an io_uring transport) is registered and can send_zc_fixed.
  // Gated to >= 64K (smaller blocks gain nothing); returns nullptr when too small / arena off /
  // exhausted, so the MIOBuffer falls back to the normal allocator. Signature matches
  // MIOBuffer::_block_alloc.
